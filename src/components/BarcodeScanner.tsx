...
const DEMO_BARCODES = [
  ...
  {
    code: '7891010031107',
    name: 'Nescafé Original Solúvel 100g',
    brand: 'Nestlé',
  },
  {
    code: '7894900011517',
...
export default function BarcodeScanner({
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setError(
        'Não foi possível acessar a câmera do dispositivo. Mas você pode digitar o código ou usar os produtos de teste abaixo!',
      );
      setCameraActive(false);
    }
  };
  ...
  const handleFetchProduct = async (barcode: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://world.openfoodfacts.org/api/v2/product/${trimmedCode}.json`,
      );
      if (!response.ok) {
      ...
      if (data.status === 1 && data.product) {
        ...
        const name =
          prod.product_name_pt || prod.product_name || 'Produto Não Nomeado';
        ...
        if (demoMatch) {
          ...
          setError(
            `Código de barras ${trimmedCode} não foi encontrado na base de dados Open Food Facts. Tente digitar outro código!`,
          );
        }
      }
    } catch (err) {
      ...
      if (demoMatch) {
        onScanSuccess(
          demoMatch.name,
          `Demonstração (offline): ${demoMatch.brand}`,
        );
      } else {
        setError(
          'Ocorreu um erro ao conectar-se à API Open Food Facts. Verifique sua conexão de rede.',
        );
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div
      ...
      class="fixed inset-0 bg-[#000000]/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4"
    >
      <div
        id="barcode-scanner-card"
        class="bg-white dark:bg-[#2c3439] w-full max-w-md rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div class="relative w-full aspect-video bg-black rounded-2xl overflow-hidden flex items-center justify-center border-4 border-[#A8DF8E] dark:border-[#433D50] shadow-inner">
            {cameraActive ? (
              ...
              <div class="text-center p-4 text-gray-400">
                ...
                <p class="text-sm">
                  Fluxo de câmera em tempo real indisponível ou permissão
                  negada.
                </p>
                <button
                  onClick={startCamera}
            ...
            <div class="flex gap-2">
              <div class="relative flex-1">
                <input
                  ...
                  class="w-full pl-9 pr-4 py-2.5 bg-gray-50 dark:bg-[#232a2d] text-gray-800 dark:text-gray-100 rounded-xl text-sm font-mono border border-transparent focus:border-[#A8DF8E] transition-all"
                />
          ...
          <div class="space-y-2 pt-2 border-t border-gray-100 dark:border-[#1b1f22]">
            <span class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1">
              <Sparkles class="w-3.5 h-3.5 text-amber-500" /> Simulador de
              Escaneamento Rápido
            </span>
            <p class="text-xs text-gray-500 dark:text-gray-400">
              Clique em um produto real brasileiro de teste abaixo para simular
              o escaneamento por câmera instantaneamente:
            </p>
            ...
            <div id="demo-barcodes-list" class="grid grid-cols-1 gap-2">
              {DEMO_BARCODES.map((item) => (
                <button
                  ...
                  class="flex items-center justify-between p-3 bg-gray-50 dark:bg-[#232a2d] hover:bg-[#A8DF8E]/20 dark:hover:bg-[#A8DF8E]/10 rounded-xl text-left transition-all border border-gray-100 dark:border-transparent group"
                >
                  <div>
        ...
        {/* Footer */}
        <div class="bg-gray-50 dark:bg-[#232a2d]/50 px-6 py-4 flex justify-between items-center border-t border-gray-100 dark:border-[#FAF8F5]/5">
